...
#else
static const std::string fonts_dir = R"(/home/enxsz/Tasks/77/)";
#endif

void PrintPDF(const movie_list& movies, std::string_view path) {
    ...
    auto font = pdf.GetFontForFile(fonts_dir + "San_Francisco.ttf");
    ...
    for (size_t i = 0; i < movies.size(); ++i) {
        ...
        if (i == 0) {
            const AbstractContentContext::TextOptions textOptions(
...

        {
            ...
            context->WriteText(left + textw + 5, top - 20 - line_height * index, " (" + std::to_string(movies[i].year) + ")", textOptions);
